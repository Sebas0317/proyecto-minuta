...
const widgetCode = `import React, { useState, useEffect, useRef } from 'react';
...
function playChime() {
  try {
    ...
    // AudioContext no permitido o silenciado
  }
}
...
export default function MinutaBotWidget() {
  ...
  const handleSuggestionClick = (sug) => {
    // Eliminar emojis iniciales si los hay
    const clean = sug.replace(/^[\\p{Emoji}\\s]+/gu, '').trim();
    handleSend(clean);
  };
...
fs.writeFileSync('c:/Users/kevin/Desktop/PROYECTOS/minuta/frontend/src/components/MinutaBotWidget.jsx', widgetCode);
console.log('✓ MinutaBotWidget.jsx written successfully');